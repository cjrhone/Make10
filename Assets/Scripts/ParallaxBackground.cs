...
public class ParallaxBackground : MonoBehaviour {
  ...
  private List<FloatingElement> elements = new();
  ...
  private void Update() {
    ...
    foreach (var element in elements) {
      ...
      var drift = driftDirection.normalized * element.speed * Time.deltaTime;

      var wobble = Mathf.Sin(time * wobbleSpeed + element.wobbleOffset) * verticalWobbleAmount * 0.01f;
      var perpendicular = new Vector2(-driftDirection.y, driftDirection.x).normalized;
      ...
      if (element.rotationSpeed != 0) {
        var currentRot = element.transform.localEulerAngles.z;
  ...
  private void GenerateElements() {
    ...
    for (var i = 0; i < midCount; i++) {
      var speed = Random.Range(midSpeedMin, midSpeedMax);
      var scale = Random.Range(midScaleMin, midScaleMax);

      if (Random.value < 0.6f) {
        var numIndex = Random.Range(0, gameNumbers.Length);
        var num = gameNumbers[numIndex];
        var tint = numberTints[numIndex];
        CreateTextElement(num, 1, scale, speed, tint, midAlpha);
      }
      else {
        var symbol = mathSymbols[Random.Range(0, mathSymbols.Length)];
        var symbolColor = new Color(0.5f, 0.5f, 0.55f);
    ...
    for (var i = 0; i < nearCount; i++) {
      var speed = Random.Range(nearSpeedMin, nearSpeedMax);
      var scale = Random.Range(nearScaleMin, nearScaleMax);
  ...
  private void CreateTextElement (string content, int layer, float fontSize, float speed, Color color, float alpha) {
    var obj = new GameObject($"Parallax_{content}_{elements.Count}");
  ...
  private void CreateSquareElement (int layer, float size, float speed, float alpha) {
    var obj = new GameObject($"Parallax_Square_{elements.Count}");
    obj.transform.SetParent(container, false);
  ...
  private void CreateEquationElement (int layer, float speed) {
    var equation = GenerateEquationThatMakesTen();

    var obj = new GameObject($"Parallax_Equation_{elements.Count}");
    obj.transform.SetParent(container, false);

    var rt = obj.AddComponent<RectTransform>();
    var fontSize = Random.Range(16f, 22f);
