...
public class Tile : MonoBehaviour, IPointerClickHandler, IBeginDragHandler, IDragHandler, IEndDragHandler {
  ...
  [Header("Number Size"), SerializeField, Range(0.5f, 2f),
  ...
  private static readonly Color TileBackgroundColor = new(0.85f, 0.85f, 0.85f);
  ...
  private static readonly Color[] NumberColors = new Color[10] {
    new(0.6f, 0.6f, 0.6f), // 0 - Grey (neutral wildcard)
    new(0.85f, 0.65f, 0.1f), // 1 - Gold (primary)
    new(0.15f, 0.4f, 0.9f), // 2 - Blue (primary)
    new(0.2f, 0.7f, 0.3f), // 3 - Green (1+2: Gold+Blue)
    new(0.85f, 0.45f, 0.35f), // 4 - Coral (warm, softer than red)
    new(0.95f, 0.5f, 0.1f), // 5 - Orange (1+4: Gold+Red)
    new(0.6f, 0.2f, 0.75f), // 6 - Purple (2+4: Blue+Red)
    new(0.1f, 0.7f, 0.7f), // 7 - Teal
    new(0.9f, 0.35f, 0.6f), // 8 - Pink
  ...
  private static readonly Color[] LockedTileColors = new Color[8] {
    new(0.94f, 0.82f, 0.37f), // 10 - Gold (#f0d060)
    new(0.82f, 0.56f, 1.00f), // 20 - Purple (#d090ff)
    new(0.31f, 0.91f, 0.82f), // 30 - Teal (#50e8d0)
    new(1.00f, 0.37f, 0.37f), // 40 - Red (#ff6060)
    new(1.00f, 0.62f, 0.19f), // 50 - Orange (#ffa030)
    new(0.37f, 0.62f, 1.00f), // 60 - Blue (#60a0ff)
    new(0.88f, 0.37f, 0.88f), // 70 - Magenta (#e060e0)
    new(0.88f, 0.37f, 0.88f) // 80+ - Magenta (same as 70)
  };
  ...
  private static Color GetLockedTileColor (int value) {
    ...
    var index = Mathf.Min(value / 10 - 1, LockedTileColors.Length - 1);
  ...
  private void CreateShineOverlay() {
    var shineObj = new GameObject("ShineOverlay");
    shineObj.transform.SetParent(transform, false);

    // Place it after the glow but before shadow/number text
    shineObj.transform.SetSiblingIndex(
      enhancedGlowImage != null ? enhancedGlowImage.transform.GetSiblingIndex() + 1 : 0);

    var shineRT = shineObj.AddComponent<RectTransform>();
  ...
  private void CreateShadowText() {
    var shadowObj = new GameObject("NumberShadow");
    ...
    var shadowRT = shadowObj.AddComponent<RectTransform>();
    // Copy the number text's rect transform settings
    var numberRT = numberText.GetComponent<RectTransform>();
  ...
  private void CreateEnhancedGlow() {
    var glowObj = new GameObject("EnhancedGlow");
  ...
  private void UpdateEnhancedGlow() {
    ...
    if (enhancedGlowImage != null) {
      if (isEnhanced) {
        ...
        var glowColor = GetLockedTileColor(Value);
    ...
    if (shadowText != null) {
      if (isEnhanced) {
        ...
        var lockedColor = GetLockedTileColor(Value);
  ...
  private IEnumerator GlowPulseCoroutine() {
    var baseNumberColor = GetTileColor();
    var brightNumberColor = Color.Lerp(baseNumberColor, Color.white, numberBrightenAmount);

    while (isEnhanced && enhancedGlowImage != null && enhancedGlowImage.gameObject.activeSelf) {
      // Glow alpha pulse
      var glowT = (Mathf.Sin(Time.time * glowPulseSpeed) + 1f) / 2f;
      var alpha = Mathf.Lerp(glowMinAlpha, glowMaxAlpha, glowT);

      var glowColor = GetTileColor();
      glowColor.a = alpha;
      enhancedGlowImage.color = glowColor;

      // Number text pulse (slightly different speed for visual interest)
      var numberT = (Mathf.Sin(Time.time * numberPulseSpeed) + 1f) / 2f;

      // Scale the number text and shadow together
      if (numberText != null) {
        var scale = Mathf.Lerp(numberPulseMinScale, numberPulseMaxScale, numberT) * numberScale;
  ...
  public void Select() {
    if (IsLocked) {
      return; // Locked tiles can't be selected
    }
  ...
  private IEnumerator AnimateDeselect() {
    ...
    while (elapsed < duration) {
      ...
      var eased = AnimationUtilities.EaseOutCubic(t);
  ...
  private IEnumerator PulseCoroutine() {
    var baseColor = TileBackgroundColor;
    var brightColor = Color.Lerp(baseColor, Color.white, 0.5f); // Brighten by 50%
    ...
    while (IsSelected) {
      ...
      var floatOffset = Mathf.Sin(Time.time * floatSpeed) * floatAmount;
      if (rectTransform != null) {
  ...
  public void OnPointerClick (PointerEventData eventData) {
    if (IsLocked) {
      return; // Locked tiles can't be clicked
    }
  ...
  public void OnBeginDrag (PointerEventData eventData) {
    if (IsLocked) {
      return; // Locked tiles can't be dragged
    }
  ...
  public void OnDrag (PointerEventData eventData) {
    ...
    var currentDelta = eventData.position - swipeStartPos;
  ...
  public void OnEndDrag (PointerEventData eventData) {
    ...
    var swipeEndPos = eventData.position;
    var swipeDelta = swipeEndPos - swipeStartPos;
  ...
  public static void RefreshAllEnhancedStatus() {
    var allTiles = FindObjectsByType<Tile>();
