...
public class SceneFlowManager : MonoBehaviour {
  ...
  private GridManager Grid {
    get {
      if (gridManagerCache == null) {
        gridManagerCache = FindAnyObjectByType<GridManager>();
      }
  ...
  private void SetupSafeArea() {
    ...
    if (canvasRect.Find("SafeAreaContainer") != null) {
      return;
    }

    // Create SafeArea container
    var safeAreaGO = new GameObject("SafeAreaContainer");
  ...
  private void InitializePanels() {
    ...
    RectTransform[] slidingPanels = {
      loadingPanel, mainMenuPanel, gamePanel,
    ...
    RectTransform[] offScreenPanels = {
      mainMenuPanel, gamePanel, optionsPanel,
  ...
  private void InitializeTutorialBuilder() {
    ...
    if (TutorialBuilder.Instance == null) {
      var builderObj = new GameObject("TutorialBuilder");
  ...
  private IEnumerator CloseOverlayToMainMenu (RectTransform overlayPanel) {
    yield return FadeTransition(overlayPanel, false);
  ...
  private IEnumerator SlideBackToMainMenu (RectTransform slidePanel) {
    yield return SlideTransition(slidePanel, mainMenuPanel, false);
    CurrentState = GameState.MainMenu;
  ...
  private IEnumerator ReturnToMainMenuFromGame() {
    ...
    yield return SlideTransition(gamePanel, mainMenuPanel, false);
    CurrentState = GameState.MainMenu;
  ...
  private IEnumerator ReturnToMainMenuFromZen() {
    ...
    yield return VerticalSlideTransition(gamePanel, mainMenuPanel, true);
    CurrentState = GameState.MainMenu;
  ...
  private IEnumerator LoadingSequence() {
    ...
    var allReady = AudioManager.Instance != null &&
    ...
    if (allReady) {
      ...
      yield return SlideTransition(loadingPanel, mainMenuPanel, true);
  ...
  private IEnumerator PlaySequence() {
    ...
    yield return SlideTransition(mainMenuPanel, gamePanel, true);
  ...
  private IEnumerator CountdownPop (bool isFinalWord = false) {
    var startScale = isFinalWord ? 1.8f : 1.5f;
    var duration = isFinalWord ? 0.2f : 0.15f;
    ...
    while (elapsed < duration) {
      ...
      var eased = isFinalWord ? AnimationUtilities.EaseOutElastic(t) : AnimationUtilities.EaseOutBack(t);
      // EaseOutBack/Elastic go from 0→1, so we map startScale→1
      var scale = Mathf.LerpUnclamped(startScale, 1f, eased);
  ...
  private IEnumerator SlideTransition (RectTransform from, RectTransform to, bool slideLeft) {
    ...
    var direction = slideLeft ? -1f : 1f;
    SetPanelPosition(to, -direction * screenWidth);

    var fromStart = from.anchoredPosition;
    var toStart = to.anchoredPosition;
    var fromEnd = new Vector2(direction * screenWidth, fromStart.y);
    var toEnd = new Vector2(0, toStart.y);
  ...
  private IEnumerator VerticalSlideTransition (RectTransform from, RectTransform to, bool slideUp) {
    ...
    var fromStart = from.anchoredPosition;
    var toStart = to.anchoredPosition;
    var fromEnd = new Vector2(fromStart.x, direction * screenHeight);
    var toEnd = new Vector2(toStart.x, 0);
  ...
  private PopupWindow GetOrCreatePopup (ref PopupWindow cached, string name) {
    if (cached == null) {
      var popupObj = new GameObject(name);
  ...
  public void OnPlayPressed() {
    ...
    HandleButton(GameState.MainMenu, () => { RunTransition(PlaySequence()); });
  }
  ...
  private void ShowZenResumePrompt() {
    ...
    var popupObj = new GameObject("ZenResumePopup");
    popupObj.transform.SetParent(mainCanvas.transform, false);
    var popup = popupObj.AddComponent<PopupWindow>();

    popup.SetTitle("MAKEZEN");
    popup.ClearContent();
    popup.SetAutoSizeMode(900f, 300f, 800f, false);

    popup.AddSpacer(16f);
    popup.AddText("SAVED GAME FOUND", UIStyleGuide.FontSizeSubheading,
      UIStyleGuide.ColorTextAccent, TextAlignmentOptions.Center, FontStyles.Bold);
    popup.AddSpacer(10f);
    popup.AddText("You have a session in progress.\nWould you like to pick up where you left off?",
      UIStyleGuide.FontSizeBody, UIStyleGuide.ColorTextSecondary, TextAlignmentOptions.Center);
    popup.AddSpacer(24f);
  ...
  private IEnumerator ZenNewGameSequence() {
    ...
    yield return VerticalSlideTransition(mainMenuPanel, gamePanel, false);
  ...
  private IEnumerator ZenResumeSequence() {
    ...
    var saveData = GameManager.Instance?.LoadZenState();
  ...
  private void ShowZenDisclaimer (System.Action onDismissed) {
    ...
    var popupObj = new GameObject("ZenDisclaimerPopup");
    popupObj.transform.SetParent(mainCanvas.transform, false);
    var popup = popupObj.AddComponent<PopupWindow>();

    popup.SetTitle("MAKEZEN");
    popup.ClearContent();
    popup.SetAutoSizeMode(900f, 400f, 1200f, false);

    // Early access notice
    popup.AddText("EARLY ACCESS", UIStyleGuide.FontSizeSubheading,
      UIStyleGuide.ColorWarning, TextAlignmentOptions.Center, FontStyles.Bold);
    popup.AddSpacer(8f);
    ...
    popup.AddText("Select any two tiles to swap them.\nTry to make rows or columns that add up to 10!",
      UIStyleGuide.FontSizeBody, UIStyleGuide.ColorTextPrimary, TextAlignmentOptions.Center);
  ...
  private IEnumerator QuitSequence() {
    yield return SlideTransition(mainMenuPanel, quitPanel, true);
    CurrentState = GameState.Quit;
    Debug.Log("Quit panel shown.");
  }

  // Legacy method - now calls GoBack()
  public void OnQuitBackPressed() {
    GoBack();
  }
  ...
  private void ShowShopComingSoonPopup() {
    var popup = GetOrCreatePopup(ref shopPopup, "ShopComingSoonPopup");
    ...
    popup.AddSpacer(10f);
    popup.AddText("Unlock avatar cosmetics\nwith your earned BP.", UIStyleGuide.FontSizeBody,
      UIStyleGuide.ColorTextSecondary,
  ...
  public void OnPausePressed() {
    if (CurrentState != GameState.Game && CurrentState != GameState.ZenGame) {
      return;
    }
  ...
  private void ShowCreditsPopup() {
    var popup = GetOrCreatePopup(ref creditsPopup, "CreditsPopup");
  ...
  private void ShowOptionsPopup() {
    var popup = GetOrCreatePopup(ref optionsPopup, "OptionsPopup");
    ...
    // --- Music Volume ---
    var currentMusic = AudioManager.Instance != null ? AudioManager.Instance.MusicVolume : 0.7f;
    popup.AddSlider("Music", currentMusic, (val) => { AudioManager.Instance?.SetMusicVolume(val); },
      UIStyleGuide.ColorButtonSecondary);

    popup.AddSpacer(10f);

    // --- SFX Volume ---
    var currentSFX = AudioManager.Instance != null ? AudioManager.Instance.SFXVolume : 1f;
    popup.AddSlider("Sound Effects", currentSFX, (val) => { AudioManager.Instance?.SetSFXVolume(val); },
      UIStyleGuide.ColorButtonPrimary);
  ...
  public bool IsInGameplay() {
    return CurrentState == GameState.Game || CurrentState == GameState.ZenGame || CurrentState == GameState.Paused;
  }
