...
public class TabletLayoutAdapter : MonoBehaviour {
  ...
  [Header("Detection"), Tooltip("Aspect ratio (short / long) above which the device is considered a tablet. " +
                                "iPad Pro 12.9\" ≈ 0.75; iPad Air 11\" ≈ 0.69; iPhone 14 ≈ 0.46. Default 0.65 " +
                                "catches common iPads while excluding every common phone."), SerializeField]
  private float portraitTabletAspectThreshold = 0.65f;

  [Header("Grid Sizing — Phone"),
   Tooltip("Fraction of parent panel WIDTH the grid may occupy on a PHONE-class aspect."), Range(0.5f, 1f),
   SerializeField]
  private float phoneWidthFillRatio = 0.92f;
  ...
  [Tooltip("Fraction of parent panel HEIGHT the grid may occupy on a PHONE-class aspect. " +
           ...
           "the height-cap kicks in and the grid shrinks to fit."), Range(0.5f, 1f), SerializeField]
  private float phoneHeightFillRatio = 0.95f;

  [Header("Grid Sizing — Tablet"),
   Tooltip("Fraction of parent panel WIDTH the grid may occupy on a TABLET-class aspect."), Range(0.5f, 1f),
   SerializeField]
  private float tabletWidthFillRatio = 0.78f;
  ...
  [Header("Tablet Extras"), Tooltip(
     "Multiplier applied to <see cref=\"additionalScaledTransforms\"/> on tablet aspect " +
     "ratios. The grid itself is sized via fill ratios, not this multiplier."), SerializeField]
  private float tabletScaleFactor = 1.15f;

  [Header("References"), Tooltip("The gameplay grid container — same RectTransform GridManager references."),
   SerializeField]
  private RectTransform gridContainer;

  [Tooltip("Optional GridManager reference. If set, RecalculateSizesFromContainer() is " +
           "called after resizing. Auto-found if null."), SerializeField]
  private GridManager gridManager;

  [Tooltip("Any additional RectTransforms to scale alongside the grid on tablet (HUD frames, " +
           "character art, decorative panels). These use the flat tabletScaleFactor."), SerializeField]
  private List<RectTransform> additionalScaledTransforms = new();

  [Header("Debug"), Tooltip("Verbose logging — leave on while tuning ratios, switch off for release."), SerializeField]
  ...
  private void ApplyGridSize() {
    ...
    var widthFill = IsTablet ? tabletWidthFillRatio : phoneWidthFillRatio;
    var heightFill = IsTablet ? tabletHeightFillRatio : phoneHeightFillRatio;

    // Width-driven attempt first
    var targetWidth = parentSize.x * widthFill;
    var targetHeight = targetWidth * gridAspect;

    // If height exceeds its cap, constrain by height instead
    var maxHeight = parentSize.y * heightFill;
    ...
    if (maxGridWidth > 0f) {
      targetWidth = Mathf.Min(targetWidth, maxGridWidth);
    }

    if (minGridWidth > 0f) {
      targetWidth = Mathf.Max(targetWidth, minGridWidth);
    }

    targetHeight = targetWidth * gridAspect;

    var before = gridContainer.sizeDelta;
  ...
  private void ApplyTabletExtras() {
    if (!IsTablet || Mathf.Approximately(UIScale, 1f)) {
      return;
    }

    if (additionalScaledTransforms == null) {
      return;
    }

    foreach (var rt in additionalScaledTransforms) {
      if (rt == null) {
        continue;
      }
  ...
  private void RefreshGridManager() {
    ...
    if (gridManager != null) {
      gridManager.RecalculateSizesFromContainer();
    }
  }
}