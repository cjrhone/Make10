...
public class HotStreakEffect : MonoBehaviour {
  ...
  [SerializeField] private Color flameEdge = new(0.9f, 0.2f, 0.1f);
  [SerializeField] private Color emberColor = new(1f, 0.6f, 0.2f);
  [SerializeField] private Color barPulseCoolColor = new(1f, 0.5f, 0.1f); // Orange
  [SerializeField] private Color barPulseHotColor = new(0.8f, 0.1f, 0.6f); // Purple/magenta
  ...
  private List<FlameParticle> flames = new();
  private List<EmberParticle> embers = new();
  private List<FlameParticle> textFlames = new();
  ...
  private void CreateFlameContainer() {
    ...
    var containerObj = new GameObject("FlameContainer");
  ...
  private void Update() {
    ...
    if (panelTransform != null && hasOriginalPosition) {
      var floatY = Mathf.Sin(time * floatSpeed) * floatAmount * currentIntensity;
      var shakeX = Mathf.Sin(time * shakeSpeed) * shakeIntensity * currentIntensity;
      var shakeY = Mathf.Cos(time * shakeSpeed * 1.3f) * shakeIntensity * 0.5f * currentIntensity;

      panelTransform.anchoredPosition = panelOriginalPosition + new Vector2(shakeX, floatY + shakeY);

      var rotWobble = Mathf.Sin(time * floatSpeed * 0.7f) * 1.5f * currentIntensity;
      panelTransform.localEulerAngles = new Vector3(0, 0, rotWobble);
    }

    // Bar color pulse - intensity affects how hot the color gets
    if (barFillImage != null) {
      var pulse = (Mathf.Sin(time * colorPulseSpeed) + 1f) / 2f;

      // Base color shifts from cool (orange) to hot (purple) based on intensity
      var baseColor = Color.Lerp(barPulseCoolColor, barPulseHotColor, currentIntensity);

      // Pulse between the base color and a brighter version
      var brightColor = Color.Lerp(baseColor, Color.white, 0.3f);
      var pulseColor = Color.Lerp(baseColor, brightColor, pulse);
  ...
  private void UpdateFlames() {
    ...
    for (var i = flames.Count - 1; i >= 0; i--) {
      var flame = flames[i];
      ...
      var lifePercent = flame.lifetime / flame.maxLifetime;
      var deathPercent = 1f - lifePercent;

      // Rise upward
      var pos = flame.transform.anchoredPosition;
      pos.y += flameRiseSpeed * Time.deltaTime;

      // Wobble side to side
      var wobble = Mathf.Sin(time * 8f + flame.wobbleOffset) * flameWobbleAmount * lifePercent;
      pos.x = flame.startX + wobble;

      flame.transform.anchoredPosition = pos;

      // Scale: grows then shrinks
      var scaleMultiplier = lifePercent < 0.3f ?
  ...
  private void UpdateEmbers() {
    ...
    for (var i = embers.Count - 1; i >= 0; i--) {
      ...
      var lifePercent = ember.lifetime / ember.maxLifetime;

      // Drift upward and sideways
      var pos = ember.transform.anchoredPosition;
      ...
      // Twinkle
      var twinkle = (Mathf.Sin(time * 20f + ember.twinkleOffset) + 1f) / 2f;
      var alpha = lifePercent * (0.5f + twinkle * 0.5f);
      ...
      // Shrink
      var scale = Mathf.Lerp(0.3f, 1f, lifePercent);
  ...
  private void UpdateTextFlames() {
    ...
    for (var i = textFlames.Count - 1; i >= 0; i--) {
      var flame = textFlames[i];
      if (flame.transform == null) {
      ...
      var scale = lifePercent * 0.8f;
      flame.transform.localScale = Vector3.one * scale;

      var c = Color.Lerp(flameEdge, flameCore, lifePercent);
      c.a = lifePercent;
      flame.image.color = c;
    }
  }
  ...
  private IEnumerator FlameSpawnerLoop() {
    while (isActive) {
      var flameCount = Mathf.RoundToInt(Mathf.Lerp(baseFlameCount, maxFlameCount, currentIntensity));

      if (flames.Count < flameCount) {
        SpawnFlame();
      }

      var adjustedRate = flameSpawnRate / (1f + currentIntensity);
  ...
  private IEnumerator EmberSpawnerLoop() {
    while (isActive) {
      var emberCount = Mathf.RoundToInt(Mathf.Lerp(baseEmberCount, maxEmberCount, currentIntensity));

      if (embers.Count < emberCount) {
        SpawnEmber();
      }

      var adjustedRate = emberSpawnRate / (1f + currentIntensity * 0.5f);
      yield return new WaitForSeconds(adjustedRate);
    }
  }
  ...
  private void SpawnFlame() {
    ...
    var flameObj = new GameObject("Flame");
    flameObj.transform.SetParent(flameContainer, false);

    var rt = flameObj.AddComponent<RectTransform>();

    // Get panel dimensions
    var panelRect = panelTransform.rect;

    // Spawn along the bottom of the panel
    var spawnX = Random.Range(-panelRect.width / 2f * 0.9f, panelRect.width / 2f * 0.9f);
    var spawnY = -panelRect.height / 2f;

    rt.anchoredPosition = new Vector2(spawnX, spawnY);

    var size = Random.Range(flameSizeRange.x, flameSizeRange.y);
    rt.sizeDelta = new Vector2(size, size * 1.5f);

    var img = flameObj.AddComponent<Image>();
    img.color = flameCore;
  ...
  private void SpawnEmber() {
    ...
    var emberObj = new GameObject("Ember");
    emberObj.transform.SetParent(flameContainer, false);

    var rt = emberObj.AddComponent<RectTransform>();

    var panelRect = panelTransform.rect;

    // Spawn around the panel area
    var spawnX = Random.Range(-panelRect.width / 2f, panelRect.width / 2f);
    var spawnY = Random.Range(-panelRect.height / 2f, panelRect.height / 4f);

    rt.anchoredPosition = new Vector2(spawnX, spawnY);

    var size = Random.Range(emberSizeRange.x, emberSizeRange.y);
    rt.sizeDelta = new Vector2(size, size);
    rt.localEulerAngles = new Vector3(0, 0, 45f); // Diamond

    var img = emberObj.AddComponent<Image>();
  ...
  private void SpawnTextFlame() {
    ...
    var flameObj = new GameObject("TextFlame");
    flameObj.transform.SetParent(multiplierText.transform, false);

    var rt = flameObj.AddComponent<RectTransform>();

    var textRect = multiplierText.rectTransform.rect;
    var spawnX = Random.Range(-textRect.width / 2f, textRect.width / 2f);
    var spawnY = textRect.height / 2f;
