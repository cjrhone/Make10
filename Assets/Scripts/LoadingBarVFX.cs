...
public class LoadingBarVFX : MonoBehaviour {
  ...
  // Internal state
  private List<RectTransform> trailParticles = new();
  private List<Image> trailImages = new();
  private List<RectTransform> shimmerBars = new();
  private Image glowOverlay;
  private float lastProgress = 0f;
  private bool isComplete = false;
  private RectTransform rectTransform;
  private readonly List<GameObject> burstParticles = new();
  ...
  private void Update() {
    if (progressSlider == null || isComplete) {
      return;
    }
  ...
  private void CreateTrailParticles() {
    for (var i = 0; i < particleCount; i++) {
      var particle = new GameObject($"TrailParticle_{i}");
  ...
  private void UpdateTrailParticles (float progress) {
    ...
    var barWidth = rectTransform.rect.width;
    var leadingEdge = progress * barWidth;

    for (var i = 0; i < trailParticles.Count; i++) {
      var rt = trailParticles[i];
      var img = trailImages[i];

      // Calculate particle position with trail effect
      var trailOffset = i / (float)particleCount * particleTrailLength;
      var particleX = leadingEdge - trailOffset;

      // Sinusoidal vertical movement - each particle has different phase
      var phase = i * 0.7f + Time.time * 4f;
      var waveAmplitude = 8f * (1f - i / (float)particleCount); // Decreasing amplitude
      var particleY = Mathf.Sin(phase) * waveAmplitude;

      // Spiral/helix pattern using Lissajous curves
      var lissajousX = Mathf.Sin(Time.time * 3f + i * 0.5f) * 3f;
      var lissajousY = Mathf.Cos(Time.time * 2f + i * 0.8f) * 5f;
      ...
      var fadeT = i / (float)particleCount;
      var alpha = Mathf.Lerp(0.9f, 0.1f, fadeT);
      ...
      var scale = Mathf.Lerp(1.2f, 0.4f, fadeT);
  ...
  private void CreateShimmerBars() {
    for (var i = 0; i < shimmerWaveCount; i++) {
      var shimmer = new GameObject($"ShimmerWave_{i}");
      shimmer.transform.SetParent(transform, false);
      shimmer.transform.SetAsFirstSibling(); // Behind particles

      var rt = shimmer.AddComponent<RectTransform>();
  ...
  private void UpdateShimmerEffect (float progress) {
    ...
    for (var i = 0; i < shimmerBars.Count; i++) {
      var rt = shimmerBars[i];
      var img = rt.GetComponent<Image>();

      // Wave position using sine - creates traveling wave effect
      var wavePhase = Time.time * shimmerSpeed + i * Mathf.PI;
      var wavePosition = (Mathf.Sin(wavePhase) + 1f) / 2f; // 0 to 1

      // Only show shimmer in the filled portion
      var shimmerX = wavePosition * progress * barWidth;

      // Gaussian-like intensity falloff
      var normalizedX = shimmerX / barWidth;

      // Create the shimmer highlight using a gradient mask approach
      // Alpha based on distance from shimmer center
      var distFromShimmer = Mathf.Abs(normalizedX - wavePosition * progress);
      var alpha = Mathf.Exp(-distFromShimmer * distFromShimmer * 50f) * shimmerIntensity;

      // Boost shimmer near leading edge
      var edgeProximity = 1f - Mathf.Abs(progress - normalizedX) * 5f;
      ...
      // Subtle scale pulse
      var scaleX = 1f + 0.02f * Mathf.Sin(Time.time * 4f + i);
  ...
  private void CreateGlowOverlay() {
    var glow = new GameObject("GlowOverlay");
  ...
  private void UpdateGlowPulse (float progress) {
    ...
    var pulse1 = Mathf.Sin(Time.time * glowPulseSpeed) * 0.5f + 0.5f;
    var pulse2 = Mathf.Sin(Time.time * glowPulseSpeed * 1.7f) * 0.3f + 0.5f;
    var pulse3 = Mathf.Sin(Time.time * glowPulseSpeed * 0.5f) * 0.2f + 0.5f;

    var combinedPulse = (pulse1 + pulse2 + pulse3) / 3f;

    // Intensity increases with progress
    var intensity = Mathf.Lerp(glowMinIntensity, glowMaxIntensity, progress);
    intensity *= combinedPulse;

    // Glow color shifts with progress
    var glowColor = GetProgressColor(progress);
  ...
  private void UpdateFillColor (float progress) {
    ...
    var targetColor = GetProgressColor(progress);

    // Add subtle hue shift based on time
    var hueShift = Mathf.Sin(Time.time * 2f) * 0.05f;
  ...
  private IEnumerator CompletionBurst() {
    ...
    var barWidth = rectTransform.rect.width;
    var burstOrigin = new Vector2(barWidth, 0);

    // Create burst particles
    for (var i = 0; i < burstParticleCount; i++) {
      var particle = new GameObject($"BurstParticle_{i}");
      particle.transform.SetParent(transform, false);
    ...
    for (var i = 0; i < burstParticleCount; i++) {
      ...
      var speed = burstSpeed * (0.5f + Random.Range(0f, 1f));
    ...
    while (elapsed < burstDuration) {
      ...
      var t = elapsed / burstDuration;

      for (var i = 0; i < burstParticles.Count; i++) {
        ...
        var gravity = new Vector2(0, -400f) * Time.deltaTime;
        ...
        // Rotate
        var rot = rt.localEulerAngles.z + rotSpeeds[i] * Time.deltaTime;
        rt.localEulerAngles = new Vector3(0, 0, rot);

        // Fade and shrink
        var alpha = 1f - t;
        var scale = Mathf.Lerp(1f, 0.2f, t);

        // Color shift during burst
        var burstColor = Color.Lerp(endColor, Color.white, t * 0.5f);
