...
public class TutorialDemoWidget : MonoBehaviour {
  ...
  [SerializeField] private Color[] numberColors = new Color[10] {
    new(0.6f, 0.6f, 0.6f), // 0 - Grey
    new(0.85f, 0.65f, 0.1f), // 1 - Gold
    new(0.15f, 0.4f, 0.9f), // 2 - Blue
    new(0.2f, 0.7f, 0.3f), // 3 - Green
    new(0.9f, 0.2f, 0.2f), // 4 - Red
    new(0.95f, 0.5f, 0.1f), // 5 - Orange
    new(0.6f, 0.2f, 0.75f), // 6 - Purple
  ...
  private void SetupDemoTiles() {
    ...
    var totalWidth = count * tileSize + (count - 1) * tileSpacing;
    var startX = -totalWidth / 2f + tileSize / 2f;

    for (var i = 0; i < count; i++) {
      // Create tile GameObject
      var tileObj = new GameObject($"DemoTile_{i}");
      tileObj.transform.SetParent(container, false);

      // Setup RectTransform
      var rt = tileObj.AddComponent<RectTransform>();
      rt.sizeDelta = new Vector2(tileSize, tileSize);
      rt.anchoredPosition = new Vector2(startX + i * (tileSize + tileSpacing), 0);

      // Add background image
      var bg = tileObj.AddComponent<Image>();
      bg.color = backgroundColor;

      // Create text child
      var textObj = new GameObject("Number");
  ...
  private IEnumerator PulseTiles() {
    ...
    for (var pulse = 0; pulse < 2; pulse++) {
      ...
      while (elapsed < 0.15f) {
        ...
        var t = elapsed / 0.15f;
        var scale = Mathf.Lerp(1f, 1.15f, t);
      ...
      while (elapsed < 0.15f) {
        elapsed += Time.deltaTime;
        var t = elapsed / 0.15f;
        var scale = Mathf.Lerp(1.15f, 1f, t);
  ...
  private IEnumerator AnimateConverge() {
    ...
    var originalPositions = new Vector2[demoTiles.Length];
    for (var i = 0; i < demoTiles.Length; i++) {
      originalPositions[i] = demoTiles[i].anchoredPosition;
    }

    var center = Vector2.zero; // Center of container

    var elapsed = 0f;
    while (elapsed < convergeDuration) {
      elapsed += Time.deltaTime;
      var t = elapsed / convergeDuration;
      var easedT = t * t * (3f - 2f * t); // Smooth step
      ...
      for (var i = 0; i < demoTiles.Length; i++) {
        ...
        // Shrink
        var scale = Mathf.Lerp(1f, 0.3f, easedT);
        ...
        if (demoTexts[i] != null) {
          var origColor = numberColors[demoValues[i]];
          var brightColor = Color.Lerp(origColor, Color.white, easedT);
  ...
  private IEnumerator ShowTenText() {
    // Create "10" text
    var tenObj = new GameObject("TenText");
    tenObj.transform.SetParent(container, false);

    var rt = tenObj.AddComponent<RectTransform>();
    ...
    while (elapsed < 0.15f) {
      elapsed += Time.deltaTime;
      var t = elapsed / 0.15f;
      var scale = Mathf.Lerp(0f, 1.2f, t);
