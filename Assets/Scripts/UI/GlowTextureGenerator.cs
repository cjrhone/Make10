...
public static class GlowTextureGenerator {
  private static Dictionary<int, Sprite> cachedGlowSprites = new();
  private static Dictionary<int, Sprite> cachedDiamondGlowSprites = new();
  ...
  public static Sprite GetCircularGlowSprite (int size = 64, float falloffPower = 2f) {
    var cacheKey = size * 100 + (int)(falloffPower * 10);

    if (cachedGlowSprites.TryGetValue(cacheKey, out var cached)) {
      return cached;
    }

    var texture = CreateCircularGlowTexture(size, falloffPower);
  ...
  public static Sprite GetDiamondGlowSprite (int size = 64, float falloffPower = 2f) {
    ...
    if (cachedDiamondGlowSprites.TryGetValue(cacheKey, out var cached)) {
      return cached;
    }

    var texture = CreateDiamondGlowTexture(size, falloffPower);
    var sprite = Sprite.Create(
      texture,
      new Rect(0, 0, size, size),
      new Vector2(0.5f, 0.5f),
  ...
  private static Texture2D CreateCircularGlowTexture (int size, float falloffPower) {
    ...
    for (var x = 0; x < size; x++) {
      ...
      var alpha = Mathf.Pow(Mathf.Max(0f, 1f - normalizedDist), falloffPower);
  ...
  private static Texture2D CreateDiamondGlowTexture (int size, float falloffPower) {
    ...
    for (var y = 0; y < size; y++)
    for (var x = 0; x < size; x++) {
      var dx = Mathf.Abs(x - center + 0.5f);
      var dy = Mathf.Abs(y - center + 0.5f);
  ...
  private static Dictionary<int, Sprite> cachedShineSprites = new();
  ...
  public static Sprite GetShineSprite (int width = 64, int height = 64, float coverage = 0.45f) {
    var cacheKey = width * 10000 + height * 10 + (int)(coverage * 100);
    if (cachedShineSprites.TryGetValue(cacheKey, out var cached)) {
      return cached;
    }

    var texture = CreateShineTexture(width, height, coverage);
    var sprite = Sprite.Create(
      texture,
