...
public class RunManager : MonoBehaviour {
  ...
  public void StartNewRun() {
    ...
    Debug.Log(
      $"<color=cyan>[RunManager] New run started! BP: {CurrentBP}, Round: {RoundNumber}, Lifetime: {TotalBP}, Spendable: {SpendableBP}</color>");
  ...
  public void BankBP (int amount) {
    ...
    var prevTotal = TotalBP;
    var prevSpendable = SpendableBP;
    ...
    Debug.Log(
      $"<color=green>[RunManager] Banked +{amount} BP! Total: {prevTotal} → {TotalBP}, Spendable: {prevSpendable} → {SpendableBP}</color>");
