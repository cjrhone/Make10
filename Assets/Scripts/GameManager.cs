...
public class GameManager : MonoBehaviour {
  ...
  public class GameSettings {
    [Header("Grid Settings")] public int gridSize = 5;
...
#pragma warning restore CS0414
  ...
  public float[] GetCurrentWeights() {
    return gameSettings.GetWeights();
  }
  ...
  public void ActivateGame() {
    CacheEffectiveValues();
    var duration = CurrentMode == GameMode.Zen ? zenGameDuration : gameDuration;
    ResetRoundState(duration);
    NotifyUIOfReset();

    Debug.Log(
      $"Game activated! Mode: {CurrentMode}, Duration: {duration}s, Grid: {gameSettings.gridSize}x{gameSettings.gridSize}");
  }
  ...
  public void OnMatchCleared (int tilesCleared, int rowsMatched, int columnsMatched, List<int> tileValues,
    MatchResult matchResult = null, int cascadeCount = 1) {
    if (!IsGameActive) {
      return;
    }

    var linesCleared = rowsMatched + columnsMatched;
    var isPlayerMatch = cascadeCount <= 1;
    ...
    if (isPlayerMatch) {
      ...
      for (var i = 0; i < linesCleared; i++) {
        var lineBaseScore = matchResult != null ? matchResult.GetLineSumByIndex(i) : baseMatchScore;
  ...
  private (int effectiveBaseScore, int enhancedBonus) CalculateCommonBonuses (List<int> tileValues) {
    var effectiveBaseScore = baseMatchScore;
    var enhancedBonus = 0;
  ...
  private void TimeUp() {
    ...
    var gamesKey = CurrentMode == GameMode.Zen ? ZEN_TOTAL_GAMES_KEY : TOTAL_GAMES_KEY;
    var hsKey = CurrentMode == GameMode.Zen ? ZEN_HIGH_SCORE_KEY : HIGH_SCORE_KEY;

    // Track total games played
    var gamesPlayed = PlayerPrefs.GetInt(gamesKey, 0) + 1;
    ...
    var endMessage = CurrentMode == GameMode.Zen ? "STILLNESS" : "TIME'S UP";
    Debug.Log(
      $"<color=cyan>*** {endMessage}! ***</color> Score: {Score} | Session: {lastSessionDuration:F1}s | Games: {gamesPlayed}");
  ...
  public void CheckAndSaveBPHighScore (int totalBP) {
    var key = CurrentMode == GameMode.Zen ? ZEN_HIGH_SCORE_BP_KEY : HIGH_SCORE_BP_KEY;
  ...
  public void SaveZenState() {
    ...
    // Tile bag state
    var twm = TileWeightManager.Instance;
    if (twm != null) {
    ...
    Debug.Log(
      $"<color=green>[Zen Save] Saved — Score: {save.score}, Time: {save.timeRemaining:F1}s, Locked: {save.zenLockedTileCount}</color>");
  }
  ...
  public ZenSaveData LoadZenState() {
    ...
    var json = PlayerPrefs.GetString(ZEN_SAVE_KEY, "");
    if (string.IsNullOrEmpty(json)) {
      return null;
    }

    var save = JsonUtility.FromJson<ZenSaveData>(json);
    if (save == null) {
      return null;
    }
    ...
    Debug.Log(
      $"<color=green>[Zen Load] Restored — Score: {Score}, Time: {TimeRemaining:F1}s, Locked: {zenLockedTileCount}</color>");
    return save;
  }
  ...
  public void ZenGameOver() {
    ...
    // Track total games
    var gamesKey = ZEN_TOTAL_GAMES_KEY;
    var gamesPlayed = PlayerPrefs.GetInt(gamesKey, 0) + 1;
    PlayerPrefs.SetInt(gamesKey, gamesPlayed);

    // Save raw score high score (legacy key)
    var hsKey = ZEN_HIGH_SCORE_KEY;
    if (Score > PlayerPrefs.GetInt(hsKey, 0)) {
      PlayerPrefs.SetInt(hsKey, Score);
    }
