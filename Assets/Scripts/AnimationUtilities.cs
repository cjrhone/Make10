...
public static class AnimationUtilities {
  ...
  /// </summary>
  public static IEnumerator ScaleIn (Transform target, float duration = 0.3f, float overshoot = 1.0f,
    ...
    while (elapsed < duration) {
      ...
      var scale = Mathf.LerpUnclamped(0f, overshoot, t);
    ...
    if (overshoot > 1f) {
      elapsed = 0f;
      var settleDuration = duration * 0.3f;
      while (elapsed < settleDuration) {
        elapsed += Time.deltaTime;
        var t = Mathf.Clamp01(elapsed / settleDuration);
        var eased = EaseOutCubic(t);
        var scale = Mathf.Lerp(overshoot, 1f, eased);
        target.localScale = Vector3.one * scale;
        yield return null;
      }
    }
  ...
  /// </summary>
  public static IEnumerator PopIn (Transform target, float overshoot = 1.2f, float popDuration = 0.2f,
  ...
  /// </summary>
  public static IEnumerator FloatAndFade (RectTransform target, TMP_Text text, float floatDistance = 50f,
    float duration = 0.8f) {
    if (target == null) {
      yield break;
    }
    ...
    var endPos = startPos + new Vector2(0, floatDistance);
  ...
  /// </summary>
  public static IEnumerator PulseLoop (Transform target, float minScale = 1.0f, float maxScale = 1.15f,
    float speed = 8f) {
    if (target == null) {
      yield break;
    }
  ...
  public static IEnumerator PulseLoopWithColor (Transform target, Graphic graphic,
    ...
    var base_c = baseColor ?? Color.white;
    var bright_c = brightColor ?? new Color(1f, 1f, 0.7f);
  ...
  /// </summary>
  public static IEnumerator CountUp (TMP_Text text, int startValue, int endValue, float duration,
    string format = "{0}") {
    if (text == null) {
      yield break;
    }
    ...
    while (elapsed < duration) {
      ...
      var currentValue = Mathf.RoundToInt(Mathf.Lerp(startValue, endValue, t));
  ...
  public static IEnumerator DropIn (RectTransform target, float dropDistance = 50f, float duration = 0.3f) {
    ...
    var startPos = endPos + new Vector2(0, dropDistance);
...
public static class AudioExtensions {
  ...
  public static void PlaySafe (this AudioManager audio, Action playAction) {
    if (audio != null) {
      playAction?.Invoke();
    }
  }
