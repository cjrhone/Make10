...
public class UIManager : MonoBehaviour {
  ...
  [SerializeField] private Color timerHealthyColor = new(0.3f, 0.8f, 0.3f);
  [SerializeField] private Color timerWarningColor = new(0.9f, 0.7f, 0.2f);
  [SerializeField] private Color timerDangerColor = new(0.9f, 0.2f, 0.2f);
...
#pragma warning restore CS0414
  ...
  private List<GameObject> zenStatLineObjects = new();
  ...
  private TMP_Text hotStreakText;
  ...
  [SerializeField] private Color multiplierGlowColor = new(1f, 0.95f, 0.5f); // Bright flash
  ...
  private void InitializeUI() {
    ...
    var isZen = gameManager != null && gameManager.CurrentMode == GameManager.GameMode.Zen;
    ...
    if (isZen && multiplierPanel != null && multiplierPanel.transform.parent != null) {
      var label = multiplierPanel.transform.parent.Find("Multiplier");
      if (label != null) {
    ...
    var initDuration = gameManager?.GameDuration ?? 60f;
  ...
  private void CreateHotStreakText() {
    ...
    var rt = hotStreakTextObject.AddComponent<RectTransform>();
    ...
    hotStreakText = hotStreakTextObject.AddComponent<TextMeshProUGUI>();
    hotStreakText.text = "HOT STREAK!";
    hotStreakText.fontSize = 72;
    hotStreakText.fontStyle = FontStyles.Bold;
    hotStreakText.alignment = TextAlignmentOptions.Center;
    hotStreakText.color = hotStreakFireColor1;

    // Enable gradient for fire effect
    hotStreakText.enableVertexGradient = true;
    hotStreakText.colorGradient = new VertexGradient(
  ...
  private void CreatePauseMenuUI() {
    ...
    var safeAreaParent = rootCanvas != null ? rootCanvas.transform.Find("SafeAreaContainer") : null;
    // Fall back to root canvas, then to UIManager itself if neither exists yet.
    var pauseParent = safeAreaParent != null ? safeAreaParent
    ...
    var hambRT = pauseHamburgerButton.AddComponent<RectTransform>();
    ...
    var hambBg = pauseHamburgerButton.AddComponent<Image>();
    ...
    var hambButton = pauseHamburgerButton.AddComponent<Button>();
    hambButton.targetGraphic = hambBg;
    var hambColors = hambButton.colors;
    ...
    hambButton.onClick.AddListener(() => { SceneFlowManager.Instance?.OnPausePressed(); });

    // Pause icon text (two bars) — sits above the fill so it reads on the dark interior
    var hambTextObj = new GameObject("HamburgerIcon");
    hambTextObj.transform.SetParent(pauseHamburgerButton.transform, false);
    var hambTextRT = hambTextObj.AddComponent<RectTransform>();
    ...
    var hambText = hambTextObj.AddComponent<TextMeshProUGUI>();
    ...
    var overlayParent = rootCanvas != null ? rootCanvas.transform : transform;
    ...
    var overlayBg = pauseOverlay.AddComponent<Image>();
    overlayBg.color = new Color(0.02f, 0.02f, 0.06f, 0.88f);

    // Content container (centered column)
    var contentContainer = new GameObject("PauseContent");
    ...
    var titleRT = pauseTitle.AddComponent<RectTransform>();
    ...
    if (scoreText != null) {
      titleText.font = scoreText.font;
    }

    // Button layout: Resume, Options, Main Menu (stacked vertically)
    var buttonWidth = 480f;
    var buttonHeight = UIStyleGuide.ButtonHeight;
    var buttonSpacing = 24f;
    var startY = -120f; // Below title
  ...
  private void CreateZenLockedTileCounter() {
    ...
    var rt = zenLockedTileCounterObj.AddComponent<RectTransform>();
  ...
  private void UpdateZenLockedTileCounter() {
    if (zenLockedTileCounterText == null || gameManager == null) {
      return;
    }
  ...
  private void UpdateTimerDisplay (float timeRemaining) {
    ...
    // Determine state
    var state = GetTimerState(timeRemaining);
    var stateColor = GetTimerColor(state);
  ...
  private void UpdateZenTimerDisplay (float timeRemaining) {
    ...
    var timeStr = minutes > 0 ? $"{minutes}:{secs:D2}" : secs.ToString();
  ...
  private void UpdateMultiplierBar (bool active, float multiplier, float barOrTimer) {
    ...
    if (active) {
      ...
      if (multiplierValueText != null) {
        ...
        var scaleT = Mathf.InverseLerp(1f, multiplierScaleAtMax, multiplier);
        var targetScale = Mathf.Lerp(multiplierMinScale, multiplierMaxScale, scaleT);

        // Color temperature: white (cool) at low multiplier, red (hot) at high
        var temperatureColor = Color.Lerp(multiplierTextCoolColor, multiplierTextHotColor, scaleT);
      ...
      if (multiplierFillImage != null && !enableHotStreak) {
        var fillPct = barOrTimer / (gameManager?.MultiplierBarMax ?? 100f);
  ...
  private IEnumerator MultiplierGlowAnimation (float targetScale, Color targetColor) {
    ...
    var textTransform = multiplierValueText.transform;
    var startScale = textTransform.localScale.x;
    var punchScale = targetScale * 1.3f; // Overshoot
    var startColor = multiplierValueText.color;

    // Phase 1: Punch up with bright glow flash
    var elapsed = 0f;
    var punchDuration = 0.15f;

    while (elapsed < punchDuration) {
      elapsed += Time.deltaTime;
      var t = elapsed / punchDuration;

      // Scale punch
      var scale = Mathf.Lerp(startScale, punchScale, t);
    ...
    elapsed = 0f;
    var settleDuration = 0.25f;

    while (elapsed < settleDuration) {
      elapsed += Time.deltaTime;
      var t = elapsed / settleDuration;
      var smoothT = 1f - Mathf.Pow(1f - t, 3f); // Ease out

      // Scale settle
      var scale = Mathf.Lerp(punchScale, targetScale, smoothT);
      textTransform.localScale = Vector3.one * scale;
  ...
  private IEnumerator HotStreakIntroSequence() {
    ...
    if (hotStreakTextObject != null) {
      ...
      var rt = hotStreakTextObject.GetComponent<RectTransform>();

      // Slide in from right
      var slideDuration = 0.4f;
      var elapsed = 0f;
      var startPos = new Vector2(800f, 0f);
  ...
  private IEnumerator HotStreakTextPulseLoop() {
    while (isInHotStreakMode && multiplierValueText != null) {
      var t = (Mathf.Sin(Time.time * hotStreakPulseSpeed) + 1f) / 2f;
      multiplierValueText.color = Color.Lerp(hotStreakFireColor1, hotStreakFireColor2, t);

      // Also pulse the scale slightly
      var scale = Mathf.Lerp(multiplierMaxScale, multiplierMaxScale * 1.1f, t);
  ...
  private void CreateHotStreakCountdownBar() {
    ...
    var avatarParent = AvatarManager.Instance != null ? AvatarManager.Instance.transform.parent : null;
    ...
    var barRect = hotStreakCountdownBarObj.AddComponent<RectTransform>();
    ...
    // Background (dark)
    var bgImage = hotStreakCountdownBarObj.AddComponent<Image>();
  ...
  private void UpdateHotStreakCountdownBar (float remainingTime) {
    if (hotStreakCountdownFillImage == null || hotStreakCountdownMax <= 0f) {
      return;
    }

    var fill = Mathf.Clamp01(remainingTime / hotStreakCountdownMax);

    // Scale the fill by adjusting anchorMax.x
    var fillRect = hotStreakCountdownFillImage.GetComponent<RectTransform>();
    if (fillRect != null) {
      fillRect.anchorMax = new Vector2(fill, 1f);
    }

    // Rainbow color cycling (matches the main multiplier bar rainbow)
    var hue = Time.time * 0.5f % 1f;
  ...
  private IEnumerator ShowFinishThenResult (bool isWin) {
    ...
    if (winScreen != null) {
      var winBg = winScreen.GetComponent<Image>();
      if (winBg != null) {
  ...
  private IEnumerator ShowWinScreenBreakdown() {
    ...
    var baseScore = gameManager.Score;
    var sessionDuration = gameManager.SessionDuration;
    var isZen = gameManager.CurrentMode == GameManager.GameMode.Zen;

    // Calculate breakdown — Zen has no session time bonus
    var sessionTimeBonus = isZen ? 0 : Mathf.RoundToInt(sessionDuration);
    var total = baseScore + sessionTimeBonus;
    ...
    var isNewHighScore = gameManager != null && gameManager.IsNewHighScore;
    ...
    var resultsTitle = isZen ? "Well Done." : "ROUND COMPLETE";

    var breakdownContainer = winScreen.transform.Find("BreakdownContainer");
    if (breakdownContainer != null && resultsTitleObj == null) {
      ...
      var titleRT = resultsTitleObj.AddComponent<RectTransform>();
      ...
      if (winScoreText != null) {
        titleText.font = winScoreText.font;
      }
    }
    ...
    if (isZen) {
      ...
      if (totalLabelText != null && totalValueText != null) {
        ...
        AudioManager.Instance?.PlayButtonClick();
        yield return StartCoroutine(AnimationUtilities.CountUp(totalValueText, 0, baseScore, countUpDuration * 1.2f,
          "{0} BP"));
      }
    }
    else {
      ...
      if (sessionTimeLabelText != null && sessionTimeValueText != null) {
        ...
        AudioManager.Instance?.PlayButtonClick();
        yield return StartCoroutine(AnimationUtilities.CountUp(sessionTimeValueText, 0, sessionTimeBonus,
          countUpDuration, "+ {0} BP"));
      }
      ...
      if (totalLabelText != null && totalValueText != null) {
        ...
        totalLabelText.text = "TOTAL";
        AudioManager.Instance?.PlayButtonClick();
        yield return StartCoroutine(AnimationUtilities.CountUp(totalValueText, 0, total, countUpDuration * 1.2f,
          "{0} BP"));
      }
    }
  ...
  private IEnumerator ShowZenStatsBreakdown (Transform breakdownContainer) {
    if (gameManager == null || breakdownContainer == null) {
      yield break;
    }

    var highestTile = gameManager.ZenHighestLockedValue;
    var matches = gameManager.ZenMatchCount;
    var chains = gameManager.ZenChainCount;
    var reshufflesUsed = gameManager.ZenMaxReshuffles - gameManager.ZenReshufflesRemaining;
    ...
    foreach (var stat in zenStats) {
      ...
      var lineObj = CreateZenStatLine(breakdownContainer, stat.label, stat.value);
  ...
  private GameObject CreateZenStatLine (Transform parent, string label, string value) {
    var lineObj = new GameObject($"ZenStat_{label}");
    ...
    if (winScoreText != null) {
      labelText.font = winScoreText.font;
    }
    ...
    if (winScoreText != null) {
      valueText.font = winScoreText.font;
    }
  ...
  private void ShowPerformanceMessage (Transform parent, string message, int stars) {
    if (performanceMessageObj != null) {
      Destroy(performanceMessageObj);
    }

    performanceMessageObj = new GameObject("PerformanceMessage");
    performanceMessageObj.transform.SetParent(parent, false);

    var msgRT = performanceMessageObj.AddComponent<RectTransform>();
    ...
    if (winScoreText != null) {
      msgText.font = winScoreText.font;
    }
  ...
  private IEnumerator ShowNewHighScoreCelebration() {
    if (newHighScoreBanner == null) {
      ...
      var bannerObj = new GameObject("NewHighScoreBanner");
      bannerObj.transform.SetParent(breakdownContainer, false);

      var bannerRT = bannerObj.AddComponent<RectTransform>();
    ...
    if (text != null) {
      var brightGold = new Color(1f, 0.95f, 0.6f);
      for (var i = 0; i < 3; i++) {
        var elapsed = 0f;
        var pulseDuration = 0.4f;
        while (elapsed < pulseDuration) {
          elapsed += Time.deltaTime;
          var t = elapsed / pulseDuration;
          var glow = Mathf.Sin(t * Mathf.PI); // 0 → 1 → 0
  ...
  private void HideBreakdownElements() {
    ...
    if (scoreLabelText != null) {
      scoreLabelText.transform.parent.gameObject.SetActive(false);
    }

    if (sessionTimeLabelText != null) {
      sessionTimeLabelText.transform.parent.gameObject.SetActive(false);
    }

    if (timeBonusLabelText != null) {
      timeBonusLabelText.transform.parent.gameObject.SetActive(false);
    }

    if (hotStreakLabelText != null) {
      hotStreakLabelText.transform.parent.gameObject.SetActive(false);
    }

    if (breakdownDivider != null) {
      breakdownDivider.gameObject.SetActive(false);
    }

    if (totalLabelText != null) {
      totalLabelText.transform.parent.gameObject.SetActive(false);
    }
    ...
    if (resultsTitleObj != null) {
      Destroy(resultsTitleObj);
      resultsTitleObj = null;
    }

    if (performanceMessageObj != null) {
      Destroy(performanceMessageObj);
      performanceMessageObj = null;
    }

    foreach (var obj in zenStatLineObjects) {
      if (obj != null) {
        Destroy(obj);
      }
    }
  ...
  private IEnumerator ShowStarRating (int starsEarned) {
    ...
    var containerRT = starContainer.AddComponent<RectTransform>();
    ...
    for (var i = 0; i < 3; i++) {
      var starObj = new GameObject($"Star_{i + 1}");
      starObj.transform.SetParent(starContainer.transform, false);

      var starRT = starObj.AddComponent<RectTransform>();
  ...
  private void EnsureBreakdownElementsExist() {
    ...
    var parent = winScreen.transform;

    // Find or create container for breakdown text
    var breakdownContainer = parent.Find("BreakdownContainer");
    if (breakdownContainer == null) {
      var containerObj = new GameObject("BreakdownContainer");
    ...
    if (sessionTimeLabelText == null || sessionTimeValueText == null) {
      (sessionTimeLabelText, sessionTimeValueText) =
        CreateBreakdownRow(breakdownContainer, "SessionTimeRow", "Session Time", "00:00");
    }

    if (timeBonusLabelText == null || timeBonusValueText == null) {
      (timeBonusLabelText, timeBonusValueText) =
        CreateBreakdownRow(breakdownContainer, "TimeBonusRow", "Time Bonus", "+ 0 BP");
    }

    if (hotStreakLabelText == null || hotStreakValueText == null) {
      (hotStreakLabelText, hotStreakValueText) =
        CreateBreakdownRow(breakdownContainer, "HotStreakRow", "Hot Streak", "x1.0");
    }
  ...
  private (TMP_Text label, TMP_Text value) CreateBreakdownRow (Transform parent, string name, string labelText,
    string valueText, bool isTotal = false) {
  ...
  private Image CreateDivider (Transform parent, string name) {
    var dividerObj = new GameObject(name);
    dividerObj.transform.SetParent(parent, false);

    var rt = dividerObj.AddComponent<RectTransform>();
    rt.sizeDelta = new Vector2(0, 6f);

    var img = dividerObj.AddComponent<Image>();
  ...
  private void EnsureResultsButtonsActive() {
    ...
    var returnMenuBtn = winScreen.transform.Find("ReturnMenuButton");
    if (returnMenuBtn != null) {
      returnMenuBtn.gameObject.SetActive(true);
    }

    var playAgainBtn = winScreen.transform.Find("PlayAgainButton");
  ...
  private TimerState GetTimerState (float timeRemaining) {
    if (timeRemaining <= timerDangerThreshold) {
  ...
  public void BounceProgressBar (float bounceScale, float duration) {
    ...
    var scaleAdd = (bounceScale - 1f) * 0.5f; // Subtler effect
  ...
  private IEnumerator SmoothBounceCoroutine() {
    var sliderTransform = scoreProgressSlider.transform;
  ...
  public void OnParticleScoreArrived (int pointsPerParticle) {
    ...
    var pointsToAdd = Mathf.Min(pointsPerParticle, pendingScoreToAdd);
  ...
  private IEnumerator FlashGlowCoroutine() {
    ...
    var elapsed = 0f;
    var startColor = scoreGlowColor;
    var endColor = new Color(scoreGlowColor.r, scoreGlowColor.g, scoreGlowColor.b, 0f);

    while (elapsed < glowFadeDuration) {
      elapsed += Time.deltaTime;
      var t = elapsed / glowFadeDuration;
  ...
  private void ActivateHotStreak (float multiplier) {
    if (!enableHotStreak || hotStreakEffect == null) {
      return;
    }
  ...
  private void UpdateHotStreakIntensity (float multiplier) {
    if (!enableHotStreak || hotStreakEffect == null || !hotStreakActive) {
      return;
    }
  ...
  private void DeactivateHotStreak() {
    if (hotStreakEffect == null || !hotStreakActive) {
      return;
    }
  ...
  private IEnumerator RainbowBarCycle() {
    var hue = 0f;
    var cycleSpeed = 0.5f; // Full cycle every 2 seconds
    ...
    while (true) {
      ...
      var rainbowColor = Color.HSVToRGB(hue, 0.85f, 1f);
  ...
  private readonly List<GameObject> activeConfetti = new();
  ...
  private static readonly Color[] confettiColors = new Color[] {
    new(1f, 0.3f, 0.35f), // Red
    new(1f, 0.85f, 0.2f), // Gold
    new(0.3f, 0.85f, 0.4f), // Green
    new(0.35f, 0.6f, 1f), // Blue
    new(0.9f, 0.4f, 0.9f), // Pink
    new(1f, 0.6f, 0.15f), // Orange
    new(0.5f, 0.85f, 1f), // Cyan
    new(0.85f, 0.7f, 1f) // Lavender
  };
  ...
  private void SpawnConfettiPiece (Transform parent, float parentWidth, float parentHeight, float duration) {
    ...
    var isSquare = Random.value > 0.6f;
    var w = Random.Range(12f, 24f);
    var h = isSquare ? w : Random.Range(20f, 40f);
    rt.sizeDelta = new Vector2(w, h);

    // Start position: spread across top, slightly above the screen
    var startX = Random.Range(-parentWidth * 0.5f, parentWidth * 0.5f);
    var startY = parentHeight * 0.5f + Random.Range(20f, 120f);
  ...
  private IEnumerator AnimateConfettiPiece (RectTransform rt, Image img,
    ...
    var elapsed = 0f;
    var pieceDuration = duration * Random.Range(0.7f, 1.0f);

    // Per-piece randomized physics
    var fallSpeed = Random.Range(400f, 700f); // pixels/sec downward
    var swayAmount = Random.Range(40f, 100f); // horizontal sway amplitude
    var swaySpeed = Random.Range(1.5f, 3.5f); // sway oscillation speed
    var spinSpeed = Random.Range(180f, 540f); // degrees/sec
    var spinAxis = Random.value; // determines which axis to spin on
    var phaseOffset = Random.Range(0f, Mathf.PI * 2f); // desync sway between pieces
    var drift = Random.Range(-60f, 60f); // gentle horizontal drift

    var startPos = rt.anchoredPosition;
    var fadeStart = pieceDuration * 0.7f; // start fading at 70% through
    ...
    while (elapsed < pieceDuration) {
      ...
      elapsed += Time.deltaTime;
      var t = elapsed / pieceDuration;

      // Vertical: accelerating fall (gravity feel)
      var y = startPos.y - fallSpeed * elapsed * (1f + t * 0.3f);

      // Horizontal: sine sway + drift
      var x = startPos.x + Mathf.Sin(elapsed * swaySpeed + phaseOffset) * swayAmount + drift * t;

      rt.anchoredPosition = new Vector2(x, y);

      // Rotation: continuous spin with flutter
      var angle = spinSpeed * elapsed;
      rt.localEulerAngles = new Vector3(0, 0, angle);

      // Simulate tumbling by squashing on one axis via scale
      var tumble = Mathf.Sin(elapsed * spinSpeed * 0.02f + phaseOffset);
      var scaleX = Mathf.Lerp(0.3f, 1f, Mathf.Abs(tumble));
      rt.localScale = new Vector3(scaleX, 1f, 1f);

      // Fade out in the last 30%
      if (elapsed > fadeStart) {
        var fadeT = (elapsed - fadeStart) / (pieceDuration - fadeStart);
        var c = img.color;
