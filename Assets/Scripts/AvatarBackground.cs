...
public class AvatarBackground : MonoBehaviour {
  ...
  private Color[] glyphTints = new Color[] {
    ...
    new(0.30f, 0.45f, 0.65f), // blue
    new(0.35f, 0.55f, 0.40f), // green
    new(0.70f, 0.40f, 0.40f), // coral
    new(0.70f, 0.50f, 0.30f), // orange
  ...
  private void SpawnGlyphs() {
    if (glyphCount <= 0 || glyphPool == null || glyphPool.Length == 0) {
      return;
    }
    ...
    for (var i = 0; i < glyphCount; i++) {
      var content = glyphPool[Random.Range(0, glyphPool.Length)];
      var size = Random.Range(glyphSizeMin, glyphSizeMax);
      var speed = Random.Range(driftSpeedMin, driftSpeedMax);

      var obj = new GameObject($"AvatarBg_{content}_{i}");
  ...
  private Color PickTintFor (string content) {
    if (content.Length == 1 && int.TryParse(content, out var digit)) {
  ...
  private void Update() {
    ...
    var t = Time.time;
    var drift = driftDirection.sqrMagnitude > 0.0001f ? driftDirection.normalized : Vector2.left;
    var perpendicular = new Vector2(-drift.y, drift.x);
    ...
    for (var i = 0; i < glyphs.Count; i++) {
      ...
      var step = drift * g.speed * Time.deltaTime;
      var wobble = Mathf.Sin(t * wobbleSpeed + g.wobbleOffset) * wobbleAmount * Time.deltaTime;
      step += perpendicular * wobble;

      g.basePosition += step;

      // Wrap around so the field is infinite within the panel bounds.
      if (g.basePosition.x < -halfW - buffer) {
        g.basePosition.x = halfW + buffer;
      }
      else if (g.basePosition.x > halfW + buffer) {
        g.basePosition.x = -halfW - buffer;
      }

      if (g.basePosition.y < -halfH - buffer) {
        g.basePosition.y = halfH + buffer;
      }
      else if (g.basePosition.y > halfH + buffer) {
        g.basePosition.y = -halfH - buffer;
      }
      ...
      if (g.rotationSpeed != 0f) {
        var rot = g.rect.localEulerAngles.z;
