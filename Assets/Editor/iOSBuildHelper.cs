...
public class iOSBuildHelper : EditorWindow {
  ...
  private void OnGUI() {
    ...
    var hasIcons = Directory.Exists(ICON_FOLDER) &&
                   Directory.GetFiles(ICON_FOLDER, "icon_*.png").Length > 0;
    var iconCount = hasIcons ? Directory.GetFiles(ICON_FOLDER, "icon_*.png").Length : 0;
    ...
    if (GUILayout.Button("Open Player Settings > iOS Icons", GUILayout.Height(28))) {
      SettingsService.OpenProjectSettings("Project/Player");
      Debug.Log(
        "[Make10] Opened Player Settings. Switch to the iOS tab, then expand the Icon section to assign icons.");
    }
    ...
    var newTeamId = EditorGUILayout.TextField("Team ID", PlayerSettings.iOS.appleDeveloperTeamID);
  ...
  private void DrawCheck (string label, string value, bool pass) {
    ...
    style.richText = true;
    var icon = pass ? "<color=#22aa22>✓</color>" : "<color=#cc3333>✗</color>";
...
#pragma warning restore CS0618
  ...
  private static void BuildiOS() {
    var buildPath = Path.Combine(Directory.GetCurrentDirectory(), BUILD_PATH);
    ...
    if (string.IsNullOrEmpty(PlayerSettings.iOS.appleDeveloperTeamID)) {
      var proceed = EditorUtility.DisplayDialog("Warning: No Team ID",
